...
| **Worker OOM / Hard Crash** | Worker process dies; job remains `running` until timeout. | **Yes** | `AnalysisReaper` runs every 30s, marks jobs with stale heartbeats (>300s) as `failed`, flips repository to `failed`. | Up to 300 seconds delay before the user sees the failure and can retry. | Decrease heartbeat timeout to 60s for small repositories; register SIGTERM handlers. |
...
| **GitHub Clone Timeout** | Git clone hangs due to network lag or massive git history. | **Yes** | `GitCloner` sets `CLONE_TIMEOUT_SECONDS=120`; terminates process and raises `CloneTimeoutError`. | Consumes 120s of worker execution time before terminating. | Implement streaming git clone progress or pre-flight HEAD checks for repo size. |
| **Oversized Repository (>100MB)** | Clone exceeds disk limit. | **Yes** | `GitCloner` monitors cloned directory size, raises `RepoTooLargeError`, marks job and repo `failed`. | Cloned bytes must be written to disk before size check aborts. | Execute GitHub API pre-check on `size` field prior to cloning. |
